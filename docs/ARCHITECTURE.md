...
Netflux is a summary-first knowledge system that turns books, podcasts, articles, and videos into structured knowledge users can search, revisit, and remember. Visitors can discover readable knowledge distilled from multiple formats without logging in. Authenticated users layer on cross-device state: saved items, reading progress, highlights, notes, reading history, and AI-powered retrieval across their library. Admin users manage publishing, series, homepage sections, media uploads, and embedding operations.
