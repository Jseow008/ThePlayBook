...
Status: In progress — cosine migration, regression proof, empty replay, and production-snapshot benchmark pass locally; production remains unchanged pending disposable-hosted verification and gated rollout
...
- Production remains unchanged and still has `vector_ip_ops`; no index, function, data, grant, policy, or migration-history mutation was made during discovery or benchmarking.
...
- [ ] The migration passes disposable-hosted verification, is deployed through the gated workflow, and passes post-deployment parity, relevance, plan, advisor, and application smoke checks.
