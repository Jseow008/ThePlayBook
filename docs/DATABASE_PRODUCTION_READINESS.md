...
- A persistent least-privilege credential is not currently available on the selected Supabase Free-plan posture. Personal Access Tokens inherit the issuing user's privileges, and the Free plan does not provide the persistent Read-Only organization role. Do not place the current broad operator token or a database/service-role credential in the scheduled job.
- A dedicated OAuth application was tested on 2026-07-29 with only `projects:read`, `database:read`, and `analytics:read`. The token successfully reached project health, read-only SQL, unified logs, backup inventory, and both advisor endpoints. The access token lifetime was 24 hours, and a controlled refresh proved that the refresh token rotates and cannot be reused as a static GitHub secret. The temporary authorization was revoked; the access token then returned HTTP 401. No Supabase access token was added to GitHub.
- `SUPABASE_PROJECT_REF` and the three verified recovery timestamps were configured in GitHub on 2026-07-29. Keep the scheduled monitor uncredentialed until a durable scoped-token custody design or a plan/role change is explicitly approved; do not work around the rotation requirement with an Owner PAT or an over-privileged GitHub credential.
