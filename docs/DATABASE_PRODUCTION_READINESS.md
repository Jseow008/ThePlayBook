...
- [`OPS.md`](./OPS.md) now defines routine zero-downtime and emergency-compromise procedures for the elevated Supabase API key, Management API token, database credential, public API key, and separate Auth signing-key lifecycle. The current server constructor can accept a named `sb_secret_...` value without renaming `SUPABASE_SERVICE_KEY`. The 2026-07-26 name/scope inventory found that Vercel exposes this variable to Production, Preview, and Development, while the GitHub production-verification secret names are not configured; splitting the Vercel scopes and deliberately provisioning the verification runner remain execution tasks, not reasons to expose or revoke the current key blindly.
