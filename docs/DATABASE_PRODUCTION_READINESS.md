...
Status: In progress — recovery targets, independent Storage verification, and a local restore drill completed on 2026-07-15; hosted retention, plan, bucket controls, and alerts remain
...
- `supabase backups list` reported no retained physical backups visible to the project, `pitr_enabled: false`, and `walg_enabled: true` on 2026-07-15. WAL archiving capability without a retained, restorable backup or PITR window does not satisfy the launch RPO.
- Both production buckets are public and currently have `file_size_limit = null` and `allowed_mime_types = null`.
- Migration `20260715110000_enforce_storage_bucket_upload_limits.sql` is prepared but not deployed. It mirrors the existing server-side upload contracts: 5 MiB with JPEG, PNG, WebP, GIF, and AVIF for `media`; 50 MiB with MPEG/MP3, WAV, and M4A MIME variants for `audio`. A live read-only audit found zero incompatible production objects. The current 79-migration sequence replayed locally and in a disposable hosted project, both pending migrations replayed idempotently in order, and all seven database/security suites passed. Production rollout remains subject to the normal backup, exact dry-run, and explicit authorization gates.
...
- **Database RPO:** no more than 24 hours of committed data loss before public launch. After a paid-plan upgrade, the target is one hour or better through PITR.
...
- **Retention:** retain at least 30 daily database and independent Storage recovery points plus 12 monthly recovery points. Keep the 2026-07-14 and 2026-07-15 pre-change backups until this automated policy has operated successfully for 30 days.
- The current manual-only database export cadence and disabled PITR do **not** meet the launch RPO. This is an explicit risk rejection, not an acceptance of the current Free-plan posture.
...
- [ ] Bucket-specific MIME-type and file-size restrictions are configured.
...
- The HNSW index uses `vector_ip_ops`.
- The active Gemini matching query uses cosine distance through `<=>`.
- A live `EXPLAIN` chooses a sequential scan and sort instead of the HNSW index.
- The sequential scan is confirmed at the current 4,088-row embedding scale, but representative latency has not yet been measured. This establishes an unused-index defect and scaling risk, not by itself a proven user-visible latency incident.
...
- [ ] The chosen metric is documented.
- [ ] If cosine similarity remains the contract, the index uses `vector_cosine_ops`.
- [ ] `EXPLAIN` shows the intended index path under representative conditions.
- [ ] Retrieval relevance is regression-tested before and after the change.
- [ ] Query latency is measured with representative library sizes.
