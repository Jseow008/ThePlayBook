...
| DB-003 | P0       | Establish production plan, backup, Storage backup, and restore readiness | Not started                                                          | Yes             |
| DB-004 | P0       | Prove a staging or disposable-environment release workflow               | In progress — hosted workflow proven; deployment enforcement pending | Yes             |
...
- Storage contains approximately 1.08 GB across 950 objects, close to or above the nominal Free allowance.
...
- [ ] Database backup retention satisfies a documented recovery point objective (RPO).
- [ ] Recovery time objective (RTO) is documented.
- [ ] PITR is enabled or explicitly risk-assessed against the required RPO.
- [ ] Audio and media objects are copied to an independent backup destination.
- [ ] Bucket-specific MIME-type and file-size restrictions are configured.
- [ ] Storage lifecycle and orphan cleanup rules are documented.
- [ ] Spend, database size, Storage size, and egress alerts are configured.
- [ ] The restore drill in [`OPS.md`](./OPS.md) is completed and recorded.

### DB-004: Prove a staging or disposable-environment release workflow

Status: In progress — disposable hosted workflow proven on 2026-07-15; production deployment enforcement remains
...
- Remaining gap: the production verification command exists, but repository/deployment settings have not yet been proven to require it as an unbypassable deployment protection rule.
...
- [ ] The production deployment requires the database verification gate to pass.
...
Current position on 2026-07-15: DB-001 and DB-002 are Verified, including the DB-002 application rollout, and DB-004's disposable hosted workflow is proven. The remaining authenticated allowlisted DB-002 user-path check is operational follow-up rather than a database-protection gate. The next schema-critical work is DB-101/DB-102, while DB-003 recovery readiness and the remaining unbypassable DB-004 deployment enforcement continue in parallel.
