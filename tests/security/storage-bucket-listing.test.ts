...
describe("storage bucket listing remediation", () => {
  const migration = readFileSync(
    join(
      process.cwd(),
      "supabase/migrations/20260624090733_restrict_public_bucket_listing.sql",
    ),
    "utf8",
  );
  ...
  it("checks for public listing drift and required admin write policies", () => {
    ...
    expect(driftCheck).toContain("storage_bucket_configuration_drift");
    expect(driftCheck).toContain("storage_object_outside_bucket_contract");
