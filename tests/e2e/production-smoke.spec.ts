...
test.describe("Production smoke checks", () => {
    ...
    test("public read page renders", async ({ page }) => {
        test.skip(!readPath, "Set SMOKE_READ_PATH to a known public read or preview path.");

        const normalizedPath = readPath!.startsWith("/") ? readPath! : `/${readPath}`;
    ...
    test("admin access is denied without a session", async ({ page }) => {
        ...
        await expect(page.getByRole("heading", { name: /content dashboard/i })).toHaveCount(0);
    });
    ...
    test("admin access is allowed for an admin session", async ({ page }) => {
        ...
        await expect(page.getByRole("heading", { name: /content dashboard|admin/i })).toBeVisible();
    });

    test("health endpoint exposes shallow and authorized detailed status", async ({ request }) => {
        test.skip(!healthSecret, "Set HEALTH_CHECK_SECRET to verify detailed health status.");

        const shallow = await request.get("/api/health");
        expect(shallow.ok()).toBe(true);
        await expect(await shallow.json()).toEqual(expect.objectContaining({ status: "ok" }));
        ...
        await expect(await detailed.json()).toEqual(expect.objectContaining({ status: "ok" }));
    });
});
