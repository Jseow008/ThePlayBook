...
const mockContentCard = vi.fn(
    ({
        ...
        <div>{`${navigationMode ?? "preview"}:${item.title}`}</div>
    )
);
...
vi.mock("@/components/ui/ContentCard", () => ({
    ContentCard: (props: { item: ContentItem; navigationMode?: "preview" | "resume" }) => mockContentCard(props),
}));
...
describe("ContinueReadingPage", () => {
    ...
    it("renders in-progress cards with resume navigation", () => {
        render(<ContinueReadingPage />);

        expect(screen.getByText("resume:Deep Work")).toBeInTheDocument();
