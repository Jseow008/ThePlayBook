...
function throwIfAborted(signal: AbortSignal | undefined) {
    if (signal?.aborted) {
        throw new AccountDataExportError("The data export was cancelled.", "EXPORT_CANCELLED");
    }
}
    ...
    try {
        ...
        for (const collection of ACCOUNT_DATA_EXPORT_COLLECTIONS) {
            ...
            do {
                ...
                const response = await fetch(url, { cache: "no-store", headers: { "Cache-Control": "no-store" }, signal });
