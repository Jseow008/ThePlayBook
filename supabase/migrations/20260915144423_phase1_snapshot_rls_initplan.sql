-- Evaluate the transaction-local account binding once per query, rather than
-- once for every protected row. This preserves the worker's scoped access
-- while resolving the Supabase RLS initplan performance advisor finding.

ALTER POLICY "Snapshot worker accesses scoped operations"
    ON snapshot_private.account_data_snapshot_operations
    USING (account_id = (SELECT NULLIF(current_setting('app.snapshot_account_id', true), '')::uuid))
    WITH CHECK (account_id = (SELECT NULLIF(current_setting('app.snapshot_account_id', true), '')::uuid));
...
ALTER POLICY "Snapshot worker accesses scoped records"
    ...
    USING (EXISTS (
        ...
        WHERE snapshot.id = snapshot_id
          AND snapshot.account_id = (SELECT NULLIF(current_setting('app.snapshot_account_id', true), '')::uuid)
    ))
    WITH CHECK (EXISTS (
        SELECT 1 FROM snapshot_private.account_data_snapshots snapshot
        WHERE snapshot.id = snapshot_id
...
ALTER POLICY "Snapshot worker reads scoped library"
    ...
    USING (user_id = (SELECT NULLIF(current_setting('app.snapshot_account_id', true), '')::uuid));
...
ALTER POLICY "Snapshot worker writes scoped library"
    ON public.user_library
    WITH CHECK (user_id = (SELECT NULLIF(current_setting('app.snapshot_account_id', true), '')::uuid));
...
ALTER POLICY "Snapshot worker reads scoped profiles"
    ON public.profiles
    USING (id = (SELECT NULLIF(current_setting('app.snapshot_account_id', true), '')::uuid));
...
ALTER POLICY "Snapshot worker reads scoped submitted requests"
    ...
    USING (submitted_by = (SELECT NULLIF(current_setting('app.snapshot_account_id', true), '')::uuid));
