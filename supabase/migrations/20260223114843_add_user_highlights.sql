-- Create user_highlights table (production-recorded version 20260223114843)
...
CREATE INDEX IF NOT EXISTS idx_user_highlights_user_content ON user_highlights(user_id, content_item_id);
...
CREATE INDEX IF NOT EXISTS segment_embedding_embedding_idx ON segment_embedding USING hnsw (embedding extensions.vector_ip_ops);
...
AS $$
  ...
  WHERE ul.user_id = p_user_id AND 1 - (se.embedding OPERATOR(extensions.<=>) query_embedding) > match_threshold
