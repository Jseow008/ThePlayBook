-- Route-only activity logging entrypoints backed by the service role.
-- This keeps browser traffic behind /api/activity/log so rate limiting and
-- server-side normalization cannot be bypassed by direct RPC calls.
