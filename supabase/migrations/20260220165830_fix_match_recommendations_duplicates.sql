...
BEGIN
    ...
    -- If no embeddings are found, fallback to category-based prioritized by latest
    IF avg_embedding IS NULL THEN
        ...
        ORDER BY
            CASE WHEN ci.category IN (
                SELECT DISTINCT category FROM public.content_item WHERE id = ANY(completed_ids) AND category IS NOT NULL
            ) THEN 0 ELSE 1 END,
            ci.created_at DESC
        LIMIT match_count;
